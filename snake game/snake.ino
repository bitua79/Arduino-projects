...
unsigned long time, timeNow, myTime; 
int gameSpeed;
...
void setup()
{
  ...
  lcd.begin(16, 2);
  pinMode(6, INPUT); //3 right
  pinMode(7, INPUT); //6 up
  pinMode(8, INPUT); //5 down
  pinMode(9, INPUT); //7 left
  pinMode(10, INPUT); //2 select
...
{
  timeNow = millis();
  if(timeNow - myTime > 1000) digitalWrite(13, LOW);
